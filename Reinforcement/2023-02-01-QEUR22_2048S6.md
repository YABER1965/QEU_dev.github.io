---
title: QEUR22_2048S6:　ゲーム2048でロバストネスを測る（その1：Julia編-MAXPOOL）
date: 2023-02-01
tags: ["QEUシステム", "メトリックス", "Julia言語", "2048", "SOART", "ディープラーニング", "強化学習", "ロバストネス"]
excerpt: julia言語とテクノメトリックスを使った強化学習とロバストネス評価
---

## QEUR22_2048S6:　ゲーム2048でロバストネスを測る（その1：Julia編-MAXPOOL）

## ～　ワタシ、「リベンジ」します　～

### ・・・　前回のつづきです　・・・

D先生(設定年齢65歳)   ： “どうします？全面的に（計算を）やり直し（をするの）？”

![imageJRL1-26-1](/2023-02-01-QEUR22_2048S6/imageJRL1-26-1.jpg)

QEU:FOUNDER(設定年齢65歳) ： “そう・・・、やりなおし。それをしないと、**総合SN比によるロバストネス評価**ができないでしょ？ロバストネスを評価して、どのテクノメトリックスが最も「とある目的に」対して有効なのかを評価したい。”

D先生： “「とある目的」？”

![imageJRL1-26-2](/2023-02-01-QEUR22_2048S6/imageJRL1-26-2.jpg)

QEU:FOUNDER ： “強化学習において、より簡単な関数を使うことが必要です。このコンセプトは、より高いスコアをとる（高性能にする）ということではないですから、気を付けて・・・。Raspberry Pi のような**軽いコンピュータでもサクサク動くこと**が本当の目的です。このような考え方が、我々のような**「衰退国家の住人」を救う道**になるかもしれないので・・・。”

D先生： “ずいぶん大きな話にしましたね。”

![imageJRL1-26-3](/2023-02-01-QEUR22_2048S6/imageJRL1-26-3.jpg)

QEU:FOUNDER ： “ほんの少し前、**EXOPLANET（地球外惑星、いわゆるパルサー）の判別**について、妙なテクノメトリックスを使って解析したよね。覚えてる？”

![imageJRL1-26-4](/2023-02-01-QEUR22_2048S6/imageJRL1-26-4.jpg)

D先生： “品質管理の世界で、「知る人ぞ知る」**CUSUMの技術**だったような・・・。”

QEU:FOUNDER ： “そうです。ああいう、**「アナログ」の技術**というのは工業化が古い国家のほうがノウハウを持っています。数学やコンピュータの世界は、すでに平準化されているので、差別化するのは「こういう分野」じゃないかなぁ・・・。それでは、BASELINEとして7つの畳み込み部品を入力として、強化学習をしてみましょう。ドン！！念を押しますが、今回はiTurn(ターン数)をディープラーニングに入力した「改良版」です！！”
...
# -----
# テクノ・メトリックスを計算する関数
function calc_techmetrics(conv_cross, mx_cvparts, mx_base, iTurn)
	...
	# MAXPOOLING -> 追加
	val_maxpool = maximum(vec(mx_cvbase))
	push!(arr_metrics, val_maxpool)
	...
	for k in 1:num_rtm
		...
		# ------
		# MAXPOOLING -> 追加
		val_maxpool = maximum(vec(mx_cvmeas))
		push!(arr_metrics, val_maxpool)
    ...
    return convert(Array{Float64}, arr_metrics)
end
...
function get_episode!(iCnt_play, memory, rep_bonuscore)
    ...
    state_cur = calc_techmetrics(conv_cross, mx_cvparts, board, iTurn)
    while flg_gameover == false        # ゲームオーバーでループから外れる
        ...
        state_next = calc_techmetrics(conv_cross, mx_cvparts, board_next, iTurn)
        ...
        #    reward = -5
        #else
        reward = round(reward + maximum(state_cur[1:4]) - minimum(state_cur[1:4]); digits=3)
        #end
...
![imageJRL1-26-5](/2023-02-01-QEUR22_2048S6/imageJRL1-26-5.jpg)

[B]

![imageJRL1-26-6](/2023-02-01-QEUR22_2048S6/imageJRL1-26-6.jpg)

**(MAXPOOL：iTurnを持つ)**

![imageJRL1-26-7](/2023-02-01-QEUR22_2048S6/imageJRL1-26-7.jpg)
...
![imageJRL1-26-8](/2023-02-01-QEUR22_2048S6/imageJRL1-26-8.jpg)

**(MAXPOOL：iTurnを持つ)**

![imageJRL1-26-9](/2023-02-01-QEUR22_2048S6/imageJRL1-26-9.jpg)

D先生  ： “あれ、Q値の絶対量が全然かわっています。フィードバック量を変えたんですか？”

QEU:FOUNDER： “後でBASELINEにつかうデータなので、極端に大きいフィードバック量にはしなかった・・・。”

D先生  ： “それでも、iTurnの導入によりQ値の成長が頭打ちにはならずに、**少しだけでも成長し続けているのはアドバンテージ**ですね。この結果を総合SN比に使うんですか？”

QEU:FOUNDER ： “まあまあ、いそがずに・・・（笑）。次はSOARTメトリックスの強化学習をやりましょう。”
...
QEU:FOUNDER ： “よくもまあ、ここまでグチャグチャにしたもんだ。”

C部長 : “「～にした」って表現、誰かが恣意的にやったようじゃないですか。”
...
QEU:FOUNDER ： “**本来は多様性のあった経済構造を自動車だけの一本足打法にして、それでコケるんだから恣意的といわれてもしょうがない**ね。そういえば、A国のT社の1台当たりの儲けは、J国のT社の8倍らしいね。”

![imageJRL1-26-10](/2023-02-01-QEUR22_2048S6/imageJRL1-26-10.jpg)

QEU:FOUNDER ： “**恩恵を受けるべき業界に「稼ぐ力」がない**のは、一体どういうこと！？さて、今回も**宮〇先生の決定版の動画**をネタにしよう。”

[![MOVIE2](http://img.youtube.com/vi/FCc8cojW3ZU/0.jpg)](http://www.youtube.com/watch?v=FCc8cojW3ZU "田内学×宮台真司：人を幸せにする経済とは")

QEU:FOUNDER ： “この（↑）中で、（彼が）言っているじゃないですか。**利権がある（産業）分野は発展しない**んだって・・・。”

C部長 : “30年間、何もしなかった分野ですね・・・。”

QEU:FOUNDER ： “**「若者の〇〇離れ」**と言われたところは、たいていそうですよ（笑）。イノベーション、イノベーションと騒いでいたが、なんだったんだ「アレ」？「利権を肥やすイノベーション」って存在しないって・・・。”

C部長 : “**利権を壊す力を持つのがイノベーション**だって・・・。昔、S社のゲーム機事業部がCDの新規格を作ったとき、音楽用のCDとの折り合いがつかなくなって二つの規格が存在したんでしたっけ・・・。”

![imageJRL1-26-11](/2023-02-01-QEUR22_2048S6/imageJRL1-26-11.jpg)

QEU:FOUNDER ： “申し遅れました。この番組は**「高齢者によるイノベーション」**です（笑）。あの旦那（↑）も、ここら辺をよくわかっているんですよね。あの産業分野のヒトだし・・・。さぁて・・・。**G国にも、もうそろそろ追い越されるかもね。彼らって「すごい」し**・・・。”
